...
export function OxymoreFeed({ onGenerate }: { onGenerate: () => void }) {
  ...
  const move = (direction: 1 | -1) => { setIndex((current) => (current + direction + examples.length) % examples.length); setLiked(false); setLikedComments([]); setCommentsOpen(false); setShareOpen(false) }
